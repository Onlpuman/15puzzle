...
type GetRandomUnique = (size: number) => number[];
type GenerateItem = (itemSize: number, id: number, positionId: number, size: number) => IItem;
type Board = (size: number) => IItem[];
...
const getRandomUnique:GetRandomUnique = (size) => {
	const array:number[] = [];
...
const generateItem:GenerateItem = (itemSize, id, positionId, size) => {
...
export const generateBoard: Board = (size) => {
	const board: IItem[] = [];
